...
def draw_case(ax, case: dict, title: str) -> list:
    ...
    xf = np.arange(0, len(truth) + 1)
    anchor = history[-1]

    ax.axvspan(0, len(truth), color=COLORS["future"], zorder=0)
    ...
    ax.set_xlim(-history_span - 1.0, len(truth) + 0.5)
    ax.set_xticks(list(history_tick_positions) + future_ticks)
...
def main() -> None:
    ...
    cases = [candidates[i - 1] for i in (2, 13, 15)]
    cases.append(cache_case(Path(args.cache_dir), sample_index=3307, sensor_index=14))
